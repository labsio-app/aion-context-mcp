...
describe('US-008 verified MCP client connection guides', () => {
  it('documents the verified client matrix and preserves the stable AION MCP contract', async () => {
    ...
    expect(appSource).toContain('portal-nav')
    expect(appSource).toContain('section-heading--tight')
    expect(appSource).toContain('client-matrix')
    expect(appSource).toContain('selectedGuideId')
    expect(appSource).toContain('copyEndpoint')
    expect(appSource).toContain('ClientGuideCard')
    expect(appSource).toContain('guide-accordion')
    expect(appSource).toContain('tool-disclosure')
    expect(appSource).toContain(':max-visible="6"')
    expect(appSource).toContain('scroll-margin-top: 24px')
